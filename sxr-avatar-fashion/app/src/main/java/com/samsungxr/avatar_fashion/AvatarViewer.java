...
public class AvatarViewer extends BaseContentScene {
    ...
    AvatarViewer(SXRContext sxrContext, Widget.OnTouchListener homeListener) {
        super(sxrContext);
        ...
        if (listProperties.length() != 0) {
            ...
            if (mAvatar != null) {
                for (SXRAnimator anim : mAvatar.getAnimations()) {
                    ...
                    animations.add(new CheckList.Item(mGvrContext, label, labelProperties, new AnimationAction(path)));
                }
            }
            mAnimList = new CheckList(mGvrContext, "Animations", animations);
            mAnimList.checkAll();
            mAnimList.enable();
    ...
    class AvatarDataLoaderWithAnimation extends Avatar.AvatarDataLoader {
        ...
        protected void onLoaded(SXRAvatar avatar, AvatarReader.Location location, String avatarName) {
            mAvatar = new Avatar(sxrContext, location, avatarName, avatar, true);
            mAvatar.setName(avatarName);
            mAvatar.setScale(Avatar.SCALE_FACTOR, Avatar.SCALE_FACTOR, Avatar.SCALE_FACTOR);
    ...
    void setAvatar(AvatarReader.Location location, String avatarName) {
        ...
        AvatarDataLoaderWithAnimation loader = (location == AvatarReader.Location.assets) ?
                new AvatarDataLoaderWithAnimation(mGvrContext, avatarsList, null) :
                new AvatarDataLoaderWithAnimation(mGvrContext, null, avatarsList);
    ...
    protected Widget createContent() {
        mContent = new ModelBox(mGvrContext, mAvatar);
        return mContent;
    }
