...
public class GamepadVirtual extends SXRSceneObject {
    ...
    private SXRTexture gamepadTexture = null;
    private float angle;
    ...
    private void attachButtons() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            GamepadButton gamepadButton = new GamepadButton(getSXRContext(), array);
    ...
    private void rotateGamepadOnY(float angle) {
        SXRRotationByAxisAnimation parentRotation = new SXRRotationByAxisAnimation(this.getParent(), 0.4f, angle, 1, 0, 0);
        parentRotation.setRepeatMode(SXRRepeatMode.ONCE);
        parentRotation.setRepeatCount(1);
        parentRotation.start(this.getSXRContext().getAnimationEngine());
    }
