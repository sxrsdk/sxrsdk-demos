...
public class Minimal360PhotoScript extends SXRMain {
    ...
    private SXRSphereSceneObject loadSkyBoxModel(SXRContext gvrContext) {
        SXRSphereSceneObject sphereObject = null;

        // load texture
        Future<SXRTexture> texture = null;
        try {
            texture = gvrContext.loadFutureTexture(new SXRAndroidResource(gvrContext, R.raw.photosphere));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private SXRTexture loadTexture(SXRContext gvrContext, String imageFile) {
        try {
            return gvrContext.loadTexture(new SXRAndroidResource(gvrContext, imageFile));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
