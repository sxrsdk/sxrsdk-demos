...
public class BulletSampleMain extends SXRMain {
    ...
    SXRCameraRig mainCameraRig = null;
    ...
    public void onInit(SXRContext gvrContext) throws Throwable {
        ...
        mContainer = new SXRSceneObject(gvrContext);
                        ...
                        if(value == null) {
                            ...
                            if (count == 10) {
                                ...
                                if (!audioPlayed || !clapPlayed) {
                                    AudioClip.getInstance(mSXRContext.getContext()).playSound(AudioClip.clapSoundID(),10,10);
                                    audioPlayed = true;
                                    clapPlayed = true;
                                }
                            }
                                ...
                                if (!audioPlayed) {
                                    AudioClip.getInstance(mSXRContext.getContext()).playSound(AudioClip.bowlingPinsHitSoundID(), 10, 10);
                                    audioPlayed = true;
                                }
                            }
    ...
    private void addCylinder(SXRScene scene, float x, float y, float z, float mass) {
    ...
    private SXRViewSceneObject createWebViewObject(SXRContext gvrContext, float w, float h, SXRView webView) {
    ...
    public void addWebViews() {
        float angle = 45f;
        SXRView webView = mActivity.getWebView(2);
        webViewObject = createWebViewObject(mSXRContext, 25f, 25f, webView);
        ...
        SXRView webView2 = mActivity.getWebView(1);
    ...
    public void addCamera(SXRSceneObject container) {
        ...
        mCameraObject.getRenderData().setRenderingOrder(SXRRenderData.SXRRenderingOrder.TRANSPARENT);
    ...
    public void togglePassthroughCamera(boolean active) {
        ...
        mCameraAnim = new SXROpacityAnimation(mCameraObject, ANIMATION_DURATION,
                active ? 1.0f : 0.0f );
